...
/**
 * DB-stored file paths (media.file_url, general.logo, general.favicon, ...) were
 * historically saved with a leading "public/" segment because the app used to be
 * served from the project root instead of the /public folder. Filesystem code
 * (File::exists/File::delete in AdminController) still relies on that stored
 * value as-is, so we don't touch the column — we only strip the prefix here,
 * at the point the path is turned into a web URL via asset().
 */

...
class OrdersController extends Controller
{
	...
	public function orders(Request $r,$status=null){
      ...
      //Filter Action End
        ...
        if($status==null || $status=='pending-payment' || $status=='unpaid' || $status=='pending' || $status=='incomplete' || $status=='confirmed' || $status=='shipped' || $status=='delivered' || $status=='cancelled'){
    ...
    public function ordersAction(Request $r,$action,$id){
        ...
        if($action=='update'){
            ...
            if(general()->mail_status && $order->email){
                ...
                $toEmail =$order->email;
                $toName =$order->name;
                $subject ='Success, Order Successfully '.$order->order_status.' '.general()->title;
